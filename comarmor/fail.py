...
from comarmor.common import error

#
# Exception handling
#
def handle_exception(*exc_info):
    '''Used as exception handler in the ca-* tools.
       For ComArmorException (used for profile syntax errors etc.), print only the exceptions
    ...
    if ex_cls.__name__  == 'ComArmorException':  # I didn't find a way to get this working with isinstance() :-/
        print('', file=sys.stderr)
        error(ex.value)
    else:
        (fd, path) = tempfile.mkstemp(prefix='comarmor-bugreport-', suffix='.txt')
        file = os.fdopen(fd, 'w')
        ...
        file.write('Please consider reporting a bug at https://github.com/ComArmor/comarmor/issues\n')
        ...
        print('Please consider reporting a bug at https://github.com/ComArmor/comarmor/issues', file=sys.stderr)
        print('and attach this file.', file=sys.stderr)

def enable_ca_exception_handler():
